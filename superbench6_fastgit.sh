...
benchinit() {
	...
	while  [ ! -e 'speedtest.py' ]; do
		...
		wget --no-check-certificate https://raw.fastgit.org/sure15/SPDTST/master/speedtest.py > /dev/null 2>&1
	done
	...
	while  [ ! -e 'speedtest_original.py' ]; do
		...
		wget --no-check-certificate https://raw.fastgit.org/sure15/SPDTST/master/speedtest_original.py > /dev/null 2>&1
	done
	...
	while  [ ! -e 'tools.py' ]; do
		...
		wget --no-check-certificate https://raw.fastgit.org/oooldking/script/master/tools.py > /dev/null 2>&1
	done
	chmod a+rx tools.py
	
	while  [ ! -e 'ip_info.py' ]; do
		echo " Installing ip_info.py ..."
		wget --no-check-certificate https://raw.fastgit.org/zzycwmx/CDN/master/ip_info.py > /dev/null 2>&1
	done
