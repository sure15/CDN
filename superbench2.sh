...
benchinit() {
	...
	while  [ ! -e 'speedtest.py' ]; do
		...
		wget --no-check-certificate https://raw.githubusercontent.com/sure15/SPDTST/master/speedtest.py > /dev/null 2>&1
	done
	...
	while  [ ! -e 'speedtest_original.py' ]; do
		...
		wget --no-check-certificate https://raw.githubusercontent.com/sure15/SPDTST/master/speedtest_original.py > /dev/null 2>&1
	done
...
speed_test(){
	if [[ $1 == '' ]]; then
		temp=$(python speedtest_original.py --single --share 2>&1)
		...
		temp=$(python speedtest.py --single --server $1 --share 2>&1)
		is_down=$(echo "$temp" | grep 'Download') 
		if [[ ${is_down} ]]; then
	        local REDownload=$(echo "$temp" | awk -F ':' '/Download/{print $2}')
	        local reupload=$(echo "$temp" | awk -F ':' '/Upload/{print $2}')
...
#     	speed_test '29071' 'Chengdu      CT'
	...
	speed_test '27810' 'Nanning      CT'
	speed_test '27154' 'TianJin      CU'
	speed_test '26180' 'Jinan        CU'
	speed_test '5145'  'Beijing      CU'
	speed_test '13704' 'Nanjing      CU'
	speed_test '5485'  'Wuhan        CU'
	speed_test '26677' 'Zhuzhou      CU'
	speed_test '9484'  'Changchun    CU'
	speed_test '4690'  'Lanzhou      CU'
	speed_test '4884'  'Fuzhou       CU'
	speed_test '31985' 'Chongqing    CU'
	speed_test '2461'  'Chengdu      CU'
	speed_test '5724'  'Hefei        CU'
	speed_test '4863'  'Xian         CU'
	speed_test '4870'  'Changsha     CU'
	speed_test '24447' 'Shanghai     CU'
	speed_test '26678' 'Guangzhou    CU'
	speed_test '16192' 'Shenzhen     CU'
	speed_test '15683' 'Nanning      CU'
	speed_test '25881' 'Jinan        CM'
	speed_test '27151' 'Linyi        CM'
	speed_test '27100' 'Qingdao      CM'
	speed_test '17184' 'Tianjin      CM'
	speed_test '34035' 'HuaiNai      CM'
	speed_test '25858' 'Beijing      CM'
	speed_test '26331' 'Zhengzhou    CM'
	speed_test '26501' 'Taiyuan      CM'
	speed_test '29105' 'Xian         CM'
	speed_test '26850' 'Wuxi         CM'
	speed_test '27249' 'Nanjing      CM'
	speed_test '17320' 'Zhenjiang    CM'
	speed_test '32291' 'Changzhou    CM'
	speed_test '26850' 'Wuxi         CM'
	speed_test '26404' 'Hefei        CM'
	speed_test '26380' 'Xian         CM'
	speed_test '25883' 'NanChang     CM'
	speed_test '28491' 'Changsha     CM'
...
ip_info4(){
	...
	echo -e " ASN & ISP            : ${SKYBLUE}$asn, $isp${PLAIN}" | tee -a $log
	echo -e " Organization         : ${YELLOW}$org${PLAIN}" | tee -a $log
	echo -e " Addr                 : ${SKYBLUE}$addr${PLAIN}" | tee -a $log
	echo -e " Location             : ${SKYBLUE}$city, ${YELLOW}$country / $countryCode${PLAIN}" | tee -a $log
	echo -e " Region               : ${SKYBLUE}$region${PLAIN}" | tee -a $log
	rm -rf tools.py
	rm -rf ip_info.py
...
print_intro() {
	...
	printf ' 添加更多节点版本: wget -qO- git.io/vpsvps | bash \n' | tee -a $log
